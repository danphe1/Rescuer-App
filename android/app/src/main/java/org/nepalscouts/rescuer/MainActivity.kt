...
class MainActivity : AppCompatActivity() {
    ...
    private val permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { grants ->
        ...
        if (locationGranted && pendingStartAfterPermission) {
            ...
            toast("Location permission is required for rescue tracking.")
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (token == null) showLogin() else restoreSession(token)
    }

    private fun showLogin(message: String? = null) {
        current = null
        root.removeAllViews()
        title("Nepal Scouts Rescuer")
        ...
        button("LOGIN", Color.rgb(5, 93, 55)) {
            ...
            lifecycleScope.launch {
                runCatching { api.login(p, code) }
                    .onSuccess { result ->
                        session.saveDeviceToken(result.token)
    ...
    private fun restoreSession(token: String) {
        ...
        lifecycleScope.launch {
            runCatching { api.status(token) }
                .onSuccess { applyStatus(it); startStatusPolling() }
                .onFailure { session.clear(); showLogin("Session expired. Please log in again.") }
        }
    }

    private fun applyStatus(status: RescueStatus) {
        current = status
        session.setMission(status.activeMissionId)
        if (status.operationalStatus !in setOf("on_mission", "returning") && session.trackingActive()) {
            TrackingController.stop(this)
        }
        renderHome()
    }
    ...
    private fun renderHome() {
        ...
        if (!approved) {
            ...
            button("REFRESH APPROVAL", Color.rgb(5, 93, 55)) { refreshStatus() }
            button("LOG OUT", Color.DKGRAY) { logout() }
            return
        }

        s.task?.let { text("Task: $it", 15f) }
        s.safeDueAt?.let { text("Next SAFE check: $it", 13f) }

        val lowData = Switch(this).apply {
            text = "Low Data Mode"
            isChecked = session.lowData()
            setOnCheckedChangeListener { _, checked -> session.setLowData(checked) }
        }
        root.addView(lowData, matchWrap())
        text("Low Data keeps GPS recording locally and batches uploads. Maps and media stay on-demand.", 12f)

        val actions = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(0, dp(10), 0, 0) }
        ...
        primaryButton(actions, "MAP", Color.rgb(18, 89, 160), true) { showMapInfo() }
        ...
        } else if (s.operationalStatus == "returning") {
            button("END MISSION & STOP TRACKING", Color.rgb(120, 35, 35)) { sendEvent("end", "Mission ended") }
        }
        button("REFRESH", Color.DKGRAY) { refreshStatus() }
        button("LOG OUT", Color.DKGRAY) { logout() }
    }
    ...
    private fun startMission() {
        ...
        lifecycleScope.launch {
            runCatching { api.event(token, "start") }
                .onSuccess { status ->
                    applyStatus(status)
                    requestPermissionsAndStart()
                }
                .onFailure { toast(it.message ?: "Could not start mission") }
        }
    }
    ...
    private fun sendEvent(event: String, success: String) {
        ...
        lifecycleScope.launch {
            runCatching { api.event(token, event) }
                .onSuccess { status -> applyStatus(status); toast(success) }
    ...
    private fun requestPermissionsAndStart() {
        ...
        } else startTrackingForCurrentMission()
    }
    ...
    private fun startTrackingForCurrentMission() {
        ...
        TrackingController.start(this, mission, session.lowData())
        toast("Foreground GPS tracking started")
        renderHome()
    }
    ...
    private fun showCheckIn() {
        ...
        AlertDialog.Builder(this).setTitle("Quick Check-In").setItems(choices) { _, which ->
            ...
            lifecycleScope.launch {
                runCatching { api.checkIn(token, choices[which].lowercase().replace(' ', '_').replace('/', '_')) }
                    .onSuccess { toast("Check-in sent") }
                    .onFailure { toast(it.message ?: "Check-in failed") }
            }
    ...
    private fun showMessage() {
        ...
        AlertDialog.Builder(this).setTitle("Message Command").setView(input).setPositiveButton("Send") { _, _ ->
            ...
            lifecycleScope.launch {
                runCatching { api.sendMessage(token, message) }
                    .onSuccess { toast("Message sent") }
                    .onFailure { toast(it.message ?: "Message failed") }
            }
        }.setNegativeButton("Cancel", null).show()
    }
    ...
    private fun showMapInfo() {
        AlertDialog.Builder(this)
            .setTitle("Operational Map")
            .setMessage("Map tiles are intentionally loaded only when the map is opened. The full native OSM map and offline route layer are the next UI layer; foreground GPS continues without map tiles.")
            .setPositiveButton("OK", null).show()
    }

    private fun sosHoldButton(parent: LinearLayout) {
        val button = Button(this).apply {
            text = "SOS — HOLD 3 SECONDS"
            setTextColor(Color.WHITE)
            setBackgroundColor(Color.rgb(190, 32, 38))
            minHeight = dp(72)
                    ...
                    MotionEvent.ACTION_DOWN -> {
                        ...
                        sosHandler.postDelayed({
                            if (sosArmed) {
                                sosArmed = false
                                text = "SOS SENT"
                                sendEvent("sos", "SOS raised — waiting for Command acknowledgement")
                            }
                        }, 3000)
                        true
                    }
    ...
    private fun refreshStatus() {
        ...
        lifecycleScope.launch {
            runCatching { api.status(token) }
                ...
                .onFailure { toast(it.message ?: "Could not refresh") }
        }
    }
    ...
    private fun startStatusPolling() {
        lifecycleScope.launch {
            while (isActive) {
                delay(if (session.lowData()) 120_000L else 30_000L)
                val token = session.deviceToken() ?: break
                runCatching { api.status(token) }.onSuccess { applyStatus(it) }
            }
        }
    }
    ...
    private fun statusCard(label: String, value: String, color: Int) {
        root.addView(TextView(this).apply { text = "$label  ·  $value"; textSize = 15f; setTextColor(Color.WHITE); setBackgroundColor(color); setPadding(dp(12), dp(10), dp(12), dp(10)) }, matchHeight(dp(50)))
    }
    private fun button(label: String, color: Int, click: (Button) -> Unit): Button = Button(this).apply {
        text = label; setTextColor(Color.WHITE); setBackgroundColor(color); minHeight = dp(52); setOnClickListener { click(this) }
        root.addView(this, matchHeight(dp(60)))
    }
    private fun primaryButton(parent: LinearLayout, label: String, color: Int, enabled: Boolean, click: () -> Unit) = parent.addView(Button(this).apply {
        text = label; isEnabled = enabled; setTextColor(Color.WHITE); setBackgroundColor(color); minHeight = dp(66); setOnClickListener { click() }
    }, matchHeight(dp(74)))
